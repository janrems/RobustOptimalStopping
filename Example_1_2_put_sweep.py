"""Example_1_2_put_sweep.py

Stress test the deep BSDE solver on the geometric American put under paired
ambiguity (Example 1.2 driver) by sweeping gamma_alpha = gamma_beta over
increasing values. Same methodology as Example_1_2_sweep.py; only the problem
setup (forward GBM in log-space + put payoff) changes.

Slightly ITM (K=1.1, S0=1) so -xi_0 = -0.1 < 0: the upper obstacle is strictly
non-trivial at t=0 across all gamma. Each (coefficient, replicate) goes to its
own folder; a top-level summary.csv and breakdown.png are produced.
"""
...
# ---- Market parameters (slightly ITM)
S0 = 1.0
K = 1.1
SIGMA_S = 0.2
R = 0.05
...
def make_problem(gamma_alpha, gamma_beta, beta_bar):
    """Closures for GBM (log-price) + Example 1.2 quadratic driver + put obstacle."""
...
def run_experiment(gamma_alpha, gamma_beta, beta_bar, run_dir, seed):
    ...
    summary["minus_Y0"] = -Y0_train if train_ok else float("nan")  # robust put price
    ...
    if train_ok and not diverged:
        try:
            ...
            for j in range(BATCH_SIZE):
                ...
                hits = diff[:-1] < tol   # exclude trivial terminal hit
                if hits.any():
            ...
            axes[0].axhline(K, color="k", linestyle=":", alpha=0.6,
                            label=f"K={K}")
            axes[0].set_title(rf"$S_t$ — $\gamma={gamma_alpha}$, seed={seed}")
...
def main():
    sweep_root = "Example_1_2_put_sweep/"
    os.makedirs(sweep_root, exist_ok=True)

    coefficients = [0.1, 0.5, 1.0, 2.0, 5.0, 10.0]
    n_replicates = 3
    beta_bar = R   # discount = risk-free rate, fixed
    ...
    for i, c in enumerate(coefficients):
        for r in range(n_replicates):
            ...
            seed = 100 * (r + 1) + 7   # paired across coefficients
    ...
    axes[0].set_ylabel(r"$-Y_0$  (robust put price)")
    axes[0].set_title(rf"K={K}, $S_0$={S0}, $\sigma$={SIGMA_S}, $r$={R}")
    axes[0].legend()
    axes[0].grid(True)
